# dte-service — emisión de DTE (SII Chile) para regsi.cl
...
[`vercel-community/php`](https://github.com/vercel-community/php)). regsi (Node) es su
único cliente y le habla por HTTP con `Authorization: Bearer <DTE_SERVICE_TOKEN>`.

La especificación completa vive en el repo de regsi:
`docs/PROMPT_FACTURACION_ELECTRONICA.md`. Este servicio es **sin estado**: los folios
viven en Supabase (los administra regsi) y el certificado llega por env var.
...
> privado de regsi, y por eso **jamás** puede haber secretos, certificados ni CAF acá.
...
| `POST /api/emitir` | stub 501 | Fase 1: genera, timbra y envía el DTE; devuelve XML + datos del timbre. **No genera PDF** (eso lo hace regsi con Chrome headless). |
...
| `DTE_SERVICE_TOKEN` | un secreto largo aleatorio | El mismo valor va en el proyecto de regsi. Generar con `openssl rand -hex 32`. |
...
- **`config.platform.ext-gd`** en `composer.json`: Vercel no trae `gd` y alguna
  ...
  resuelva, porque **el camino de PDF de LibreDTE no se usa**: el PDF lo genera regsi.
